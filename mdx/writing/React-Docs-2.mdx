...
###### Capture 이벤트 핸들러로 (심지어 stopPropagation을 실행해도) 이벤트 캡쳐링시에 이벤트를 실행할 수 있다.
...
###### 이벤트 핸들러는 side effects를 위한 최적의 장소다.
...
###### 컴포넌트는 여러 정보들을 “기억”해야 하는데, 리액트에서 컴포넌트에 특화된 메모리를 state라 부른다.
...
###### Hook은 리액트가 렌더링 될때만 사용가능한 특수한 함수다.
...
###### 리액트의 state는 어떻게 상태(무엇을 리턴할지)를 기억하는가?
...
###### Props와 달리 state는 오직 해당 컴포넌트(state를 선언한) 내부에서만 관리할 수 있다.
